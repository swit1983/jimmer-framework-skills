...
| 模式 | 说明 | 适用场景 | 特点 |
|------|------|---------|------|
| `AUTO` (默认) | **自动判断** - 根据对象是否有 ID 判断 INSERT 或 UPDATE | 大多数场景 | 最常用，智能判断 |
| `INSERT_ONLY` | **强制 INSERT** - 无论是否有 ID，都执行 INSERT | 确定是新增数据 | ID 通常由数据库生成 |
| `UPDATE_ONLY` | **强制 UPDATE** - 无论是否有 ID，都执行 UPDATE | 确定是修改数据 | 要求对象必须有 ID，否则报错 |
| `UPSERT` | **原生 UPSERT** - 利用数据库原生 UPSERT 能力 | 批量操作、高性能场景 | 依赖数据库支持（如 MySQL 的 ON DUPLICATE KEY UPDATE） |
| `INSERT_IF_ABSENT` | **不存在才 INSERT** - 先检查是否存在，不存在才插入 | 幂等插入、防止重复数据 | 根据 key 或 id 判断是否存在 |

### 各模式详细行为

| 模式 | 对象有 ID | 对象无 ID | 行为 |
|------|----------|----------|------|
| `AUTO` | 执行 UPDATE | 执行 INSERT | 智能判断 |
| `INSERT_ONLY` | 忽略 ID，执行 INSERT | 执行 INSERT | 强制新增 |
| `UPDATE_ONLY` | 执行 UPDATE | **抛出异常** | 强制更新，无 ID 报错 |
| `UPSERT` | UPSERT（存在则更新，不存在则插入） | 执行 INSERT | 原生批量操作 |
| `INSERT_IF_ABSENT` | 按 ID 检查存在性，不存在则插入 | 按 key 检查存在性，不存在则插入 | 幂等插入 |

### 代码示例

```java
// AUTO 模式（默认）- 根据是否有 ID 自动判断
Book newBook = BookDraft.$.produce(draft -> {
    draft.setName("New Book");  // 无 ID，将执行 INSERT
});
Book savedNew = sqlClient.save(newBook);

Book existingBook = BookDraft.$.produce(draft -> {
    draft.setId(1L);  // 有 ID，将执行 UPDATE
    draft.setName("Updated Book");
});
Book savedExisting = sqlClient.save(existingBook);
...
// UPDATE_ONLY 模式 - 强制 UPDATE
Book forcedUpdate = BookDraft.$.produce(draft -> {
    draft.setId(1L);  // 必须有 ID，否则抛出异常
    draft.setName("Force Update");
});
Book updated = sqlClient
    .saveCommand(forcedUpdate)
    .setMode(SaveMode.UPDATE_ONLY)
    .execute()
    .getModifiedEntity();

// UPSERT 模式 - 原生批量操作（高性能）
List<Book> books = Arrays.asList(book1, book2, book3);
BatchSaveResult<Book> result = sqlClient
    .saveCommand(books)
    .setMode(SaveMode.UPSERT)  // 使用数据库原生 UPSERT
    .execute();

// INSERT_IF_ABSENT 模式 - 幂等插入（不存在才插入）
Book idempotentBook = BookDraft.$.produce(draft -> {
    draft.setName("Unique Book");  // 假设 name 是 key
    // 无 ID，将按 key 检查是否存在
});
...
| 快捷方法 | 默认 SaveMode | 说明 |
|---------|--------------|------|
| `save()` / `saveEntities()` / `saveInputs()` | `AUTO` | 自动判断 INSERT 或 UPDATE |
| `insert()` / `insertEntities()` / `insertInputs()` | `INSERT_ONLY` | 强制 INSERT |
| `insertIfAbsent()` / `insertEntitiesIfAbsent()` / `insertInputsIfAbsent()` | `INSERT_IF_ABSENT` | 不存在才 INSERT |
| `update()` / `updateEntities()` / `updateInputs()` | `UPDATE_ONLY` | 强制 UPDATE |
| `upsert()` / `upsertEntities()` / `upsertInputs()` | `UPSERT` | 原生 UPSERT |
| `merge()` / `mergeEntities()` / `mergeInputs()` | `AUTO` | 等同于 save |
