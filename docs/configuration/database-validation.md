...
```
JSqlClient sqlClient = JSqlClient    .newBuilder()    .setDatabaseValidationMode(        DatabaseValidationMode.ERROR    )    ...省略其他配置...    .build();
```


```
val sqlClient = newKSqlClient {    setDatabaseValidationMode(        DatabaseValidationMode.ERROR    )    ...省略其他配置...}
```
...
```
jimmer:  database-validation:     mode: ERROR    catalog: db1
```
...
```
JSqlClient sqlClient = JSqlClient    .newBuilder()    .setDatabaseValidationMode(        DatabaseValidationMode.ERROR    )    .setDatabaseValidationCatalog("db1")    ...省略其他配置...    .build();
```


```
val sqlClient = newKSqlClient {    setDatabaseValidationMode(        DatabaseValidationMode.ERROR    )    setDatabaseValidationCatalog("db1")    ...省略其他配置...}
```
...
```
jimmer:      database-validation:         mode: ERROR        catalog: mydatabase        schema: myschema
```
- 底层API配置
...
```
JSqlClient sqlClient = JSqlClient    .newBuilder()    .setDatabaseValidationMode(        DatabaseValidationMode.ERROR    )    .setDatabaseValidationCatalog("mydatabase")    setDatabaseValidationSchema("myschema")    ...省略其他配置...    .build();
```


```
val sqlClient = newKSqlClient {    setDatabaseValidationMode(        DatabaseValidationMode.ERROR    )    setDatabaseValidationCatalog("mydatabase")    setDatabaseValidationSchema("myschema")    ...省略其他配置...}
```
...
- 修饰实体接口，表示整个实体不用验证。
- 修饰实体属性，表示特定属性不用验证。
[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/configuration/database-validation.mdx)最后 于 **2025年9月16日**  更新
- [功能介绍](#功能介绍)
- [开启验证](#开启验证)
- [解决表冲突](#解决表冲突)
- [暂时忽略局部验证](#暂时忽略局部验证)