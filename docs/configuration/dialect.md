...
```
@Beanpublic Dialect dialect() {    return new MySqlDialect();}
```


```
@Beanfun dialect(): Dialect =    MySqlDialect()
```
...
```
JSqlClient sqlClient = JSqlClient    .newBuilder()    .setDialect(new MySqlDialect())    ...省略其他配置...    .build();
```


```
val sqlClient = newKSqlClient {    setDialect(MySqlDialect())    ...省略其他配置...}
```
...
[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/configuration/dialect.mdx)最后 于 **2025年9月16日**  更新
- [设置方言](#设置方言)
- [方言列表](#方言列表)