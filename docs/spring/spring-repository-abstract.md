...
因此，请参考<https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#repository-query-keywords>，本文不再赘述。

## 动态WHERE[​](#动态where "动态WHERE的直接链接")
...
无论是本文讨论的简单查询，还是下一篇文档要讨论的[复杂查询](/zh/docs/spring/repository/default)，
只要查询返回实体对象或其集合，而非简单的列元组，
都建议添加一个`Fetcher`参数，让所有对象查询具备如同`GraphQL`一样的强大数据结构形状控制能力。

这会为上层业务开发带来巨大的便利。

[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/spring/repository/abstract.mdx)
