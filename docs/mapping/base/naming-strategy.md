...
```
@Beanpublic DatabaseNamingStrategy databaseNamingStrategy() {    return DefaultDatabaseNamingStrategy.LOWER_CASE;}
```


```
@Beanfun databaseNamingStrategy(): DatabaseNamingStrategy =    DefaultDatabaseNamingStrategy.LOWER_CASE
```
...
```
JSqlClient sqlClient = JSqlClient    .newBuilder()    .setDatabaseNamingStrategy(        DefaultDatabaseNamingStrategy.LOWER_CASE    )    ...省略其他配置...    .build();
```


```
val sqlClient = newKSqlClient {    setDatabaseNamingStrategy(        DefaultDatabaseNamingStrategy.LOWER_CASE    )    ...省略其他配置...}
```

[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/mapping/base/naming-strategy.mdx)最后 于 **2025年9月16日**  更新
- [命名策略接口](#命名策略接口)
- [默认命名策略](#默认命名策略)
- [UPPER_CASE](#upper_case)
- [LOWER_CASE](#lower_case)
- [覆盖策略](#覆盖策略)
- [使用SpringBoot时](#使用springboot时)
- [不使用SpringBoot时](#不使用springboot时)