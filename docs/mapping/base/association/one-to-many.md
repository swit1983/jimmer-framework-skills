...
- 左侧：在[多对一](mapping/base/association/many-to-one)一文中我们讨论过的`Book.store`
...
```
@Entitypublic interface Book {    @ManyToOne    @JoinColumn(name = "STORE_ID")    BookStore store();    ...省略其他代码...}
```

Book.kt

```
@Entityinterface Book {    @ManyToOne    @JoinColumn(name = "STORE_ID")    val store: BookStore    ...省略其他代码...}
```
...
```
@Entitypublic interface BookStore {    // `mappedBy`表示`BookStore.books    // `是`Book.store`的镜像    @OneToMany(mappedBy = "store")    List<Book> books();    ...省略其他代码...}
```

BookStore.kt

```
@Entityinterface BookStore {    // `mappedBy`表示`BookStore.books`    // 是`Book.store`的镜像    @OneToMany(mappedBy = "store")    val books: List<Book>    ...省略其他代码...}
```
...
[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/mapping/base/association/one-to-many.mdx)最后 于 **2025年9月16日**  更新