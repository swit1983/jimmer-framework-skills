...
> 来源: https://jimmer.deno.dev/zh/docs/mapping/base/nullity
...
## 定义属性的可空性[​](#定义属性的可空性 "定义属性的可空性的直接链接")

### Kotlin[​](#kotlin "Kotlin的直接链接")
...
* 当以下任何一种情况满足时，一对一和多对一属性必须可null
  ...
  + 远程关联
  + 作为镜像端的@OneToOne关联
* 如果关联实体被施加了[全局过滤器](/zh/docs/query/global-filter)，那么，对于一对多/多对一关联而言，即使直接基于外键也必须可null，除非以下条件全部满足：
...
## inputNotNull[​](#inputnotnull "inputNotNull的直接链接")
...
如果某个关联属性配置了`inputNotNull`，在[数据库验证](/zh/docs/configuration/database-validation)中会被优先考虑，从而验证数据库中外键字段不能为null。

[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/mapping/base/nullity.mdx)
