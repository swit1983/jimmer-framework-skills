...
> 来源: https://jimmer.deno.dev/zh/docs/cache/multiview-cache/user-filter
...
在[全局过滤器](/zh/docs/query/global-filter/user-filter)一文中，我们介绍了自定义全局过滤器需要实现`Filter/KFilter`接口。
...
- 任何一个全局过滤器对缓存不友好，都会导致对此过滤器敏感的其他属性都无法支持缓存
  ...
  如果不小心导致这种无意义的混用，Jimmer会告诉[为什么缓存未生效](/zh/docs/cache/multiview-cache/abandoned-callback)。
- 当所有全局过滤器都缓存友好时，所有`CacheableFilter/KCacheableFilter`对象的`getParameters()`方法返回的数据合并在一起，作为多视图缓存的`SubKey`

  例如，实体同时被两个全局过滤器处理。一个是[逻辑删除](/zh/docs/query/global-filter/logical-deleted)所隐含的过滤器，记为`a`；另外一个是用户自定义过滤器，记为`b`。
...
### 定义缓存友好过滤器[​](#定义缓存友好过滤器 "定义缓存友好过滤器的直接链接")
...
## 启用多视图缓存[​](#启用多视图缓存 "启用多视图缓存的直接链接")

### 普通的写法[​](#普通的写法 "普通的写法的直接链接")
...
## 计算属性的SubKey[​](#计算属性的subkey "计算属性的SubKey的直接链接")
...
- 第二步：通过关联缓存查询❶处的`BookStore.books`
  ...
  上面的代码会得到一系列聚合根对象，如果数据库采用官方例子的数据，会得到两个聚合根对象，这两条BOOK\_STORE的主键`ID`为1和2。
...
### 多种身份反复执行[​](#多种身份反复执行 "多种身份反复执行的直接链接")
...
swagger界面的支持，用三种不同的用户身份执行三次很容易。
...
- ❹ id为1的`BookStore`对象的计算缓存`BookStore.avgPrice`也被影响了，这是最奇妙的特征。
  ...
  虽然框架并不知道用户在计算属性中使用何种计算方法，但是在[计算缓存](/zh/docs/cache/cache-type/calculation)一文中，我们在`BookStoreAvgPriceResolver`类中讨论过如下代码
...
```
  ...
  如果忘记了这段代码的具体逻辑，可以回顾[计算缓存](/zh/docs/cache/cache-type/calculation)一文，这里关注高量的那行代码即可。这里，当前计算属性对`BookStore.books`关联的变化感兴趣。
...
[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/cache/multiview-cache/user-filter.mdx)
