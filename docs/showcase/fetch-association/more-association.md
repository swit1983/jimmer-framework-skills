...
```
BookTable table = BookTable.$;List<Book> books = sqlClient    .createQuery(table)    .where(table.name().eq("Learning GraphQL"))    .orderBy(table.edition().desc())    .select(        table.fetch(            BookFetcher.$                .allScalarFields()                .store( ❶                    BookStoreFetcher.$                        .allScalarFields()                )                .authors( ❷                    AuthorFetcher.$                        .allScalarFields()                )        )    )    .execute();
```


```
val books = sqlClient    .createQuery(Book::class) {        where(table.name  eq "Learning GraphQL")        orderBy(table.edition.desc())        select(            table.fetchBy {                allScalarFields()                store { ❶                    allScalarFields()                }                authors { ❷                    allScalarFields()                }            }        )    }    .execute()
```
...
```
export com.yourcompany.yourproject.model.Book    -> package com.yourcompany.yourproject.model.dtoBookView {    #allScalars(this)    store { ❶        #allScalars(this)    }    authors { ❷        #allScalars(this)    }}
```
...
[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/showcase/fetch-association/more-association.mdx)最后 于 **2025年9月16日**  更新
- [查询动态实体](#查询动态实体)
- [查询静态DTO](#查询静态dto)