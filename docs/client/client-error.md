...
> 来源: https://jimmer.deno.dev/zh/docs/client/error
...
然而，既然提供了[前后端API免对接](/zh/docs/client/api)，那么就必须提供这部分功能，否则就不是一套完整的方案。
...
### 自动生成异常簇[​](#自动生成异常簇 "自动生成异常簇的直接链接")

Jimmer采用枚举来定义异常簇，枚举也是这簇异常的错误码，例如

#### 定义异常簇[​](#定义异常簇 "定义异常簇的直接链接")
...
这部分类型已经在[概述/快速上手/生成代码](/zh/docs/quick-view/get-started/generate-code)一节中讨论过，此处不在重复。
...
#### 为异常码添加字段[​](#为异常码添加字段 "为异常码添加字段的直接链接")
...
### 手写异常簇[​](#手写异常簇 "手写异常簇的直接链接")

【TODO】

## 为REST API声明异常[​](#为rest-api声明异常 "为REST API声明异常的直接链接")

### 允许抛出整个异常簇[​](#允许抛出整个异常簇 "允许抛出整个异常簇的直接链接")
...
### 允许抛出异常簇中部分异常[​](#允许抛出异常簇中部分异常 "允许抛出异常簇中部分异常的直接链接")
...
### 将异常消息写入HTTP响应[​](#将异常消息写入http响应 "将异常消息写入HTTP响应的直接链接")
...
### 生成的TypeScript代码[​](#生成的typescript代码 "生成的TypeScript代码的直接链接")
...
| USER\_INFO\_ERROR\_CODE |      | PLATFORM\_ERROR\_CODE  |               |
|-------------------------|------|------------------------|---------------|
| ---                     | ---  | ---                    | ---           |
| ILLEGAL\_USER\_NAME     | Used | PERMISSION\_DENIED     | Used          |
| PASSWORD\_TOO\_SHORT    | Used | DATA\_IS\_FROZEN       | **Not used!** |
| PASSWORD\_NOT\_SAME     | Used | SERVICE\_IS\_SUSPENDED | Used          |
...
### TypeScript IDE的效果[​](#typescript-ide的效果 "TypeScript IDE的效果的直接链接")
...
- `ApiErrors["userController"]`下面有三个选项: `login`、`logout`和`registerUser`
  ...
  ![](/zh/assets/images/api-2a7966c9db85aec24ae0d1fe0cb62c17.webp)
- `ApiErrors["userController"]["registerUser"]`的`family`字段有两个选项: `USER_INFO_ERROR_CODE`和`PLATFORM_ERROR_CODE`

  ![](/zh/assets/images/family-b9c1857991f5f8d662444aa9f6e2b85f.webp)
- 一旦`family`被确定为`UserInfoErrorCode`，`code`字段有三个选项: `ILLEGAL_USER_NAME`、`PASSWORD_TOO_SHORT`和`PASSWORDS_NOT_SAME`

  ![](/zh/assets/images/code-bfdfb727fb3ccc54d12fe6a4c5ddc58f.webp)
- 一旦`code`被确定为`ILLEGAL_USER_NAME`，则可以使用`illegalChars`附加字段

  ![](/zh/assets/images/field-05834b18f87cfe96cfd1d1a72750e9e5.webp)

[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/client/error.mdx)
