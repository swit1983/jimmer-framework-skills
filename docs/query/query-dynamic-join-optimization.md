...
> 来源: https://jimmer.deno.dev/zh/docs/query/dynamic-join/optimization
...
### 另外一种解决方案[​](#另外一种解决方案 "另外一种解决方案的直接链接")

在

目前技术方案存在问题中，我们列举了两个场景。

在[上一篇文档](/zh/docs/query/dynamic-join/merge)中，我们讨论了利用Jimmer SQL DSL自动合并冲突连接的特性解决这两个场景的问题。
...
* 解决场景2的问题
  ...
  + 这里的`asTableEx` *(以及Java下的各种TableEx类型)* 是后续文档[分页安全性](/zh/docs/query/dynamic-join/table-ex)要介绍的概念。这里，请读者先忽略它。

## 幻连接[​](#幻连接 "幻连接的直接链接")

### 判断关联id的两种方法[​](#判断关联id的两种方法 "判断关联id的两种方法的直接链接")
...
1. 直接使用外键属性
   ...
   * Kotlin
   ...
   \*即使用户不为`Book`实体声明[@IdView](/zh/docs/mapping/advanced/view/id-view)属性，
...
* 这两种方法，并不等价
  ...
  + 如果关联对象受[全局过滤器](/zh/docs/query/global-filter)影响，即使数据库中存在关联对象，也应该视而不见。
* 然而，当以下所有条件同时满足时，上述两种方法是完全等价的。

  + 关联属性是基于外键的一对一或多对一关联，而非基于中间表，或其他关联的反向映射。
  + 外键是真的，即数据中存在对应的外键约束。请参见[真假外键](/zh/docs/mapping/base/foreignkey)
  + 关联对象不受[全局过滤器](/zh/docs/query/global-filter)影响。

    注意：这里指以下两种过滤器中任何一种

    - [用户自定义过滤器](/zh/docs/query/global-filter/user-filter)
    - [逻辑删除过滤器](/zh/docs/query/global-filter/logical-deleted)
...
### 判断关联id的两种方法[​](#判断关联id的两种方法-1 "判断关联id的两种方法的直接链接")
...
1. 判断中间表中的外键字段
   ...
   * Kotlin
   ...
   例子中的`asTableEx`，并无实质性的功能，将会在下一篇文档[分页安全性](/zh/docs/query/dynamic-join/table-ex)中做介绍。这里，请读者先忽略它。

   警告

   对于一对多/多对多关联而言 \*(这个例子的`Book.authors`)而言，
   \*用户必须`Book`实体声明[@IdView](/zh/docs/mapping/advanced/view/id-view)属性，
...
* 然而，当以下所有条件同时满足时，上述两种方法是完全等价的。
  ...
  + 中间表中的外键是真的，即数据中存在对应的外键约束。请参见[真假外键](/zh/docs/mapping/base/foreignkey)
  + 关联对象不受[全局过滤器](/zh/docs/query/global-filter)影响。
...
[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/query/dynamic-join/optimization.mdx)
