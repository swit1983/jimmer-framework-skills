...
> 来源: https://jimmer.deno.dev/zh/docs/query/paging/usage
...
## Jimmer分页的特色[​](#jimmer分页的特色 "Jimmer分页的特色的直接链接")
...
Jimmer不但可以自动生成`count-query`，还可以做到优化`count-query`。这个优化会在[下一篇文章](/zh/docs/query/paging/unnecessary-join)中讨论。

## 配合Spring Data使用时[​](#配合spring-data使用时 "配合Spring Data使用时的直接链接")
...
* 按照一定的约定声明抽象方法，交由Jimmer自动实现
  ...
  你可以查看[Spring篇/SpringData风格/抽象方法](/zh/docs/spring/repository/abstract)以了解如何通过这种方式实现分页查询
...
## 不配合Spring Data使用时[​](#不配合spring-data使用时 "不配合Spring Data使用时的直接链接")

### Jimmer的Page对象[​](#jimmer的page对象 "Jimmer的Page对象的直接链接")
...
为了简化讨论，这段伪码并未考虑[反排序优化](/zh/docs/query/paging/reverse-sorting)
...
* ❷ 在不修改原样板查询的基础上，生成`count-query`，再执行`count-query`得到分页前所有数据的行数
  ...
  Jimmer不但能自动生成`count-query`，还能自动优化`count-query`，请参见[表连接优化](/zh/docs/query/paging/unnecessary-join)
...
### MySqlDialect[​](#mysqldialect "MySqlDialect的直接链接")
...
### OracleDialect[​](#oracledialect "OracleDialect的直接链接")
...
## 和对象抓取器配合[​](#和对象抓取器配合 "和对象抓取器配合的直接链接")
...
[编辑此页](https://github.com/babyfish-ct/jimmer-doc/edit/main/i18n/zh/docusaurus-plugin-content-docs/current/query/paging/usage.mdx)
